...
//
{
  function removeEmpty(content) {
    return content.filter(item => item !== null && typeof item !== 'undefined');
  };
...
ElementValue
  = chars:([^<\n\r]+)
  {
    return createNode('#text', {}, escapeSource.html(escapeSource.str(chars.join(''))));
  }
...
Cdata "CDATA"
  = '<![CDATA[' children:CdataContent
    {
      return createNode('#cdata', {}, escapeSource.str(children));
    }
CdataContent
  = ']]>'
...
Comment "comment"
  = '<!--' children:CommentContent
    {
      return createNode('#comment', {}, children);
    }
CommentContent
  = '-->' { return '' }
...
FestValue "fest:value"
  = '<fest:value>' children:FestValueContent {
      return createNode('fest:value', {}, escapeSource.str(children));
    }
FestValueContent
  = '</fest:value>' { return '' }
  / head:. tail:FestValueContent { return head + tail }


FestText "fest:text"
  = '<fest:text>' children:FestTextContent {
      // return createNode('fest:text', {}, escapeSource.html(escapeSource.str(children)));
      return createNode('#text', {}, escapeSource.html(escapeSource.str(children)));
    }
FestTextContent
  = '</fest:text>' { return '' }
  / head:. tail:FestTextContent { return head + tail }


FestParams "fest:params"
  = '<fest:params>' content:FestParamsContent {
      const children = content.split('\n').map(x => x.trim()).join('');
      try {
        new Function(`return (${children})`);
      } catch (e) {
        throw e;
      }
      return createNode('fest:params', {}, children);
    }
FestParamsContent
  = '</fest:params>' { return '' }
  / head:. tail:FestParamsContent { return head + tail }
